...
class TodoResource(Resource):
    ...
    def get(self):
        ...
        self._logger.info('calling controller\'s get method')
        todo_list = self.controller.read()
        self._logger.info(f'controller finished and returned: {todo_list}')
    ...
    def post(self):
        ...
        self._logger.info(f'calling POST with body: {body}')
        try:
            self._logger.info(f'calling controller\'s post method')
            new_todo = self.controller.create(**body)
            self._logger.info(f'controller finished and returned: {new_todo}')
